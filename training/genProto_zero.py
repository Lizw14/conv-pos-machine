...
def setLayers(data_source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False):
    ...
    n.pool_center_lower = L.Pooling(n.center_map, kernel_size=5, stride=4, pool=P.Pooling.AVE)
...
def getSolverPrototxt(base_lr, folder_name, stepsize):
    string = 'net: "prototxt/MPI_LEEDS_alltrain/pose_train_test.prototxt"\n\
...
if __name__ == "__main__":
    ...
    directory = 'prototxt/MPI_LEEDS_alltrain'
    dataFolder = '/lmdb/MPI_LEEDS_alltrain'
    ...
    base_lr = 8e-5
    transform_param = dict(stride=8, crop_size_x=184, crop_size_y=184,
                             target_dist=1.171, scale_prob=1, scale_min=0.7, scale_max=1.3,
                             max_rotate_degree=40, center_perterb_max=0, do_clahe=False, num_parts=14, np_in_lmdb=14, is_dataset_zero=True)
    nCP = 2
    CH = 128
    stage = 3
