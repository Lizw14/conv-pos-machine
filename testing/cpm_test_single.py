...
class Config:
    def __init__(self):
        ...
        self.caffemodel = '../training/prototxt/trainval_finetune2/caffemodel/pose_iter_40000.caffemodel'
        self.deployFile = '../training/prototxt/trainval_finetune2/pose_deploy.prototxt'
...
def applymodel(net, oriImg, param, rectangle, watchstage=2):
    ...
    t0 = time.time()
    score = applydnn(imageToTest, net, nstage, param.boxsize)
...
if __name__ == '__main__':
    ...
    # test a folder
    imageDir = '/home/tianlu/deephi_data/'
    resultDir = 'result_test_finetune/'
    textFile = open('pose_test.txt', 'r')
    ...
    for i in xrange(len(lines)):
        ...
        heatMaps, prediction = applymodel(net, image, param, rectangle, watchstage=2)
        if param.npoints == 14:
            draw_joints(test_image, prediction, save_image)
        if param.npoints == 13:
